...
The client sends data to the server using TCP sockets. It creates a socket with
AF_INET and SOCK_STREAM, connects to 127.0.0.1:8001, and sends messages using
write().

2. What data is the client sending to the server?
The client sends five string messages: "Hello", "Apple", "Car", "Green", and
"Dog".

Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
The run_acceptor thread receives a pointer to a struct acceptor_args. This
struct contains a run flag, a pointer to the shared list_handle used to store
messages, and a pointer to the mutex that protects the shared list.

2. How are received messages stored?
Received messages are stored in a linked list. For each received message, the
server creates a new list_node, allocates memory for the message, copies the
message data into it, and appends the node to the shared list.

3. What does `main()` do with the received messages?
main() waits until the expected number of messages has been received, stops the
acceptor thread, joins it, checks whether enough messages were received, then
traverses the linked list, prints the collected messages, and frees the nodes.

4. How are threads used in this sample code?
One thread runs the acceptor loop to accept incoming client connections. A
separate thread is created for each connected client, allowing multiple clients
to send messages to the server in parallel.

Explain the use of non-blocking sockets in this lab.
Non-blocking sockets are used so threads do not get stuck forever in accept() or
read(). Sockets are made non-blocking using fcntl() with the O_NONBLOCK flag.
The listening server socket and each client socket are made non-blocking. This
lets the threads repeatedly check their run flags and shut down cleanly instead
of blocking forever waiting for a connection or incoming data.
*/
